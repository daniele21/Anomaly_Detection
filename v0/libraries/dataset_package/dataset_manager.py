...
def checkAnomaly(patch, mask):
    ...
    y = patch.y_range
#    print(x,y)
#    print(mask[y[0]:y[1], x[0]:x[1]])
#    print('Limit Patch: ', x,y)
    anomaly = mask[y[0]:y[1], x[0]:x[1]]
#    print('Anomaly shape', anomaly.shape)
#    print('Anomaly: \n', anomaly)
    
    n_anom_pixels = np.count_nonzero(anomaly)
    nPixels = patch.shape.x * patch.shape.y
    anom_perc = (n_anom_pixels / nPixels) * 100
    
#    print('Anomaly pixels:\t{}/{}'.format(n_anom_pixels, nPixels))
#    print('\tAnomaly Perc: \t{} %'.format(anom_perc))
    
    if(anom_perc > THRESHOLD_ANOMALY*100):
#        print('Anomaly pixels:\t{}/{}'.format(n_anom_pixels, nPixels))
#        print('\tAnomaly Perc: \t{} %'.format(anom_perc))
        return True
    else:
